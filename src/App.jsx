...
export default function App() {
  ...
  const totalMinDebt = debts.reduce((sum, d) => sum + Number(d.min_payment || d.minPayment || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-start py-6 px-4">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col border border-gray-100 relative pb-20">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-900 text-white rounded-2xl p-4 space-y-3 shadow-md">
                ...
                <div className="border-t border-slate-800 pt-2 flex justify-between items-center">
                  ...
                  <span className={`text-sm font-black ${disposableIncome >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>TSH {disposableIncome.toLocaleString()}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 border border-slate-100 p-3.5 rounded-2xl">
                  ...
                  <span className={`text-sm font-extrabold ${disposableIncome >= 0 ? 'text-slate-900' : 'text-rose-600'}`}>TSH {disposableIncome.toLocaleString()}</span>
                      ...
                      {isEditing ? (
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-1">
                            ...
                            <input 
                              type="number" 
                              value={editDebtForm.min_payment} 
                              onChange={(e) => setEditDebtForm({ ...editDebtForm, min_payment: e.target.value })} 
                              className="px-2 py-1.5 border rounded-lg text-xs" 
                              placeholder="Min Pay"
                            />
                          </div>
                          <div className="grid grid-cols-3 gap-1">
                            <input 
                              ...
                              onChange={(e) => setEditDebtForm({ ...editDebtForm, apr: e.target.value })} 
                              className="px-2 py-1.5 border rounded-lg text-xs" 
                              placeholder="APR %"
                            />
                            <input 
                              type="number" 
                              value={editDebtForm.duration_months} 
                              onChange={(e) => setEditDebtForm({ ...editDebtForm, duration_months: e.target.value })} 
                              className="px-2 py-1.5 border rounded-lg text-xs" 
                              placeholder="Duration"
                            />
                            <input 
                              type="number" 
                              value={editDebtForm.due_day} 
                              onChange={(e) => setEditDebtForm({ ...editDebtForm, due_day: e.target.value })} 
                              className="px-2 py-1.5 border rounded-lg text-xs" 
                              placeholder="Due Day"
                            />
                          </div>
          ...
          {activeTab === 'payoff' && (
            <div className="space-y-4">
              ...
              <div className="bg-emerald-50 border border-emerald-100 p-3.5 rounded-2xl flex justify-between items-center">
                ...
                <span className={`text-xs font-black ${disposableIncome >= 0 ? 'text-emerald-900' : 'text-rose-600'}`}>TSH {disposableIncome.toLocaleString()}</span>
              </div>
                    ...
                    return (
                      <div key={debt.id} className="bg-white border border-gray-100 p-3 rounded-xl flex justify-between items-center text-xs">
                        <div>
                          ...
                          <div className="text-[10px] text-gray-400">Min: TSH {minPay.toLocaleString()} {index === 0 && disposableIncome > 0 && <span className="text-emerald-600 font-bold ml-1">(+ Extra Target)</span>}</div>
                        </div>
                        <span className="font-extrabold text-slate-800">
                          TSH {(minPay + (index === 0 ? Math.max(0, disposableIncome) : 0)).toLocaleString()}
                        </span>
                      </div>
                    );
                  })}
          ...
          {activeTab === 'profile' && (
            <div className="space-y-4">
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-4 space-y-3">
                ...
                <div>
                  <label className="text-[10px] text-gray-500 uppercase font-semibold">Emergency Buffer Target</label>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-4 space-y-3">
                ...
                <form onSubmit={handleAddEssential} className="space-y-2">
                  <input type="text" placeholder="Essential Name (e.g., Insurance, Subscriptions)" value={newEssentialName} onChange={(e) => setNewEssentialName(e.target.value)} className="w-full px-3 py-1.5 border rounded-lg text-xs" required />
                  ...
                  <button type="submit" className="w-full py-2 bg-slate-900 text-white rounded-lg text-xs font-bold">Add Custom Essential</button>
                </form>

                <div className="space-y-2 pt-2">
                  {customEssentials.map(item => (
                    <div key={item.id} className="flex justify-between items-center bg-slate-50 border border-gray-100 p-2.5 rounded-xl text-xs">
                      <span className="font-medium text-slate-800">{item.name}</span>
                      <div className="flex items-center space-x-2">
                        <span className="font-bold text-rose-600">- TSH {item.amount.toLocaleString()}</span>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 py-3 px-6 flex justify-between items-center text-xs font-semibold text-gray-400">
          <button onClick={() => setActiveTab('shield')} className={`flex flex-col items-center space-y-1 transition ${activeTab === 'shield' ? 'text-emerald-600 font-bold' : 'hover:text-slate-600'}`}>
            <span className="text-base">🛡️</span>
            <span className="text-[10px]">Shield</span>
          </button>
          <button onClick={() => setActiveTab('debts')} className={`flex flex-col items-center space-y-1 transition ${activeTab === 'debts' ? 'text-emerald-600 font-bold' : 'hover:text-slate-600'}`}>
            <span className="text-base">💳</span>
            <span className="text-[10px]">Debts</span>
          </button>
          <button onClick={() => setActiveTab('payoff')} className={`flex flex-col items-center space-y-1 transition ${activeTab === 'payoff' ? 'text-emerald-600 font-bold' : 'hover:text-slate-600'}`}>
            <span className="text-base">📈</span>
            <span className="text-[10px]">Payoff</span>
          </button>
          <button onClick={() => setActiveTab('profile')} className={`flex flex-col items-center space-y-1 transition ${activeTab === 'profile' ? 'text-emerald-600 font-bold' : 'hover:text-slate-600'}`}>
            <span className="text-base">⚙️</span>
